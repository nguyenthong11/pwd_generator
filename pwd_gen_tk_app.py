...
class GUI:
    def __init__(self):
        ...
        self.output_pw = tk.Entry(
            self.passwd, textvariable=self.value, bd=0, bg="systemButtonFace"
        )
        ...
        self.copy = tk.Button(
            self.button, text="Copy to clipboard", command=self.clipper
        )
        self.quit = tk.Button(
            self.button, text="Quit", command=self.main_window.destroy
        )
    ...
    def clipper(self) -> None:
        ...
        self.main_window.after(
            2000, lambda: self.copied_label.config(text="")
        )  # Hide after 2s
